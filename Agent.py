...
class Agent:
    ...
    def Solve(self, problem):
        ...
        if self.is3x3:
            ...
            images = [image_a, image_b, image_c, image_d, image_e, image_f, image_g, image_h]
            image_1, image_2, image_3, image_4, image_5, image_6, image_7, image_8 = self.Load_Problem_Answer_Images()
            answer_images = [image_1, image_2, image_3, image_4, image_5, image_6, image_7, image_8]
            ...
            AND_3x3_Answer_DPR, AND_3x3_Difference = self.Get_AND_3x3_Answer_DPR(images)
            OR_3x3_Answer_DPR, OR_3x3_Difference = self.Get_OR_3x3_Answer_DPR(images)
            ...
            Subtract_3x3_Answer_DPR, Subtract_3x3_Difference = self.Get_Subtract_3x3_Answer_DPR(images)
            ...
            Diagonal_Similarity_3x3_Answer_DPR, Diagonal_Similarity_3x3_Difference = self.Check_Diagonal_Similarity_3x3_DPR(images + answer_images)
            Diagonal_XOR_Similarity_3x3_Answer_DPR, Diagonal_XOR_Similarity_3x3_Difference = self.Check_Diagonal_XOR_Similarity_3x3_DPR(images + answer_images)

            answer_list = [AND_3x3_Answer_DPR, OR_3x3_Answer_DPR, Subtract_3x3_Answer_DPR, Diagonal_Similarity_3x3_Answer_DPR, Diagonal_XOR_Similarity_3x3_Answer_DPR]
            # score_list = [AND_3x3_Score, OR_3x3_Score, XOR_3x3_Score, Subtract_3x3_Score]
            difference_list = [AND_3x3_Difference, OR_3x3_Difference, Subtract_3x3_Difference, Diagonal_Similarity_3x3_Difference, Diagonal_XOR_Similarity_3x3_Difference]
            for d, index in zip(difference_list, range(len(difference_list))):
                if d == 0:
                    difference_list[index] = 100
            transformations = ['AND Images DPR', 'OR Images DPR', 'Subtract Images DPR', 'Diagonal Similarity DPR', 'Diagonal XOR Similarity DPR']

            # answer = answer_list[score_list.index(max(score_list))]
            print('Final DPR Results:', transformations, answer_list, difference_list)
            ...
            if answer != default_answer:
                ...
                AND_3x3_Answer_IPR, AND_3x3_Score = self.Get_AND_3x3_Answer_IPR(images)
                OR_3x3_Answer_IPR, OR_3x3_Score = self.Get_OR_3x3_Answer_IPR(images)
                XOR_3x3_Answer_IPR, XOR_3x3_Score = self.Get_XOR_3x3_Answer_IPR(images)
                Simple_XOR_3x3_Answer_IPR, Simple_XOR_3x3_Score = self.Get_Simple_XOR_3x3_Answer_IPR(images)
                Subtract_3x3_Answer_IPR, Subtract_3x3_Score = self.Get_Subtract_3x3_Answer_IPR(images)

                Diagonal_Similarity_3x3_Answer_IPR, Diagonal_Similarity_3x3_Score = self.Check_Diagonal_Similarity_3x3_IPR(images + answer_images)
                Diagonal_XOR_Similarity_3x3_Answer_IPR, Diagonal_XOR_Similarity_3x3_Score = self.Check_Diagonal_XOR_Similarity_3x3_IPR(images + answer_images)

                answer_list = [AND_3x3_Answer_IPR, OR_3x3_Answer_IPR, XOR_3x3_Answer_IPR, Simple_XOR_3x3_Answer_IPR, Subtract_3x3_Answer_IPR, Diagonal_Similarity_3x3_Answer_IPR, Diagonal_XOR_Similarity_3x3_Answer_IPR]
                score_list = [AND_3x3_Score, OR_3x3_Score, XOR_3x3_Score, Simple_XOR_3x3_Score, Subtract_3x3_Score, Diagonal_Similarity_3x3_Score, Diagonal_XOR_Similarity_3x3_Score]

                transformations = ['AND Images IPR', 'OR Images IPR', 'XOR Images IPR', 'Simple XOR Images IPR', 'Subtract Images IPR', 'Diagonal Similarity IPR', 'Diagonal XOR Similarity IPR']
                print('Final IPR Results:', transformations, answer_list, score_list)
                answer = answer_list[score_list.index(max(score_list))]
                if answer != default_answer:
                    print('Solution found with `%s` . . . \n' % transformations[score_list.index(max(score_list))])
                    if ENV == 'dev':
            ...
            while solution_found == False and t_list_index < len(transformations_list):
                transformation_string = self.Transformation_Dict()[str(transformations_list[t_list_index])]
                ...
                if 'Difference' in transformation_string:
                    ...
                    image_b_added_diff = self.OR_images(image_b, find_difference_AC)
                    image_c_added_diff = self.OR_images(image_c, find_difference_AB)
                    image_b_removed_diff = self.AND_images(image_b, find_difference_AC)
                    image_c_removed_diff = self.AND_images(image_c, find_difference_AB)
                    print('Adding Difference')
                    print('Looking for a match for C*(A-B) to D or B*(A-C) to D . . . \n')
                    for i in range(1, self.num_options+1):
                        print('Testing Answer %d . . . ' % i)
                        image_d = self.Load_Image(str(i))
                        score_diff_h = self.PixelCompare(image_c_added_diff, image_d, transformations_list[t_list_index], transformations_done)
                        score_diff_v = self.PixelCompare(image_b_added_diff, image_d, transformations_list[t_list_index], transformations_done)
                        if score_diff_h > score_diff_v:
                            print('C*(A-B) to D is a better match with a score of %f > %f' % (score_diff_h, score_diff_v))
                            if score_diff_h > threshold:
                                answer = i
                                solution_found = True
                                self.Print_Found_Match(answer)
                            else:
                                print('%f is lower than the the threshold %f . . . ' % (score_diff_h, threshold))
                        else:
                            print('B*(A-C) to D is a better match with a score of %f > %f' % (score_diff_v, score_diff_h))
                    ...
                    for i in range(1, self.num_options+1):
                        print('Testing Answer %d . . . ' % i)
                        image_d = self.Load_Image(str(i))
                        score_diff_h = self.PixelCompare(image_c_removed_diff, image_d, transformations_list[t_list_index], transformations_done)
                        score_diff_v = self.PixelCompare(image_b_removed_diff, image_d, transformations_list[t_list_index], transformations_done)
                        if score_diff_h > score_diff_v:
                            print('C+(A-B) to D is a better match with a score of %f > %f' % (score_diff_h, score_diff_v))
                            if score_diff_h > threshold:
                                answer = i
                            ...
                            if score_diff_v > threshold:
                                ...
                                print('%f is lower than the the threshold %f . . . ' % (score_diff_h, threshold))
                    break
                print('Checking which relationship is stronger: A to B or A to C . . . \n')
                print('-- A to B --')
                if transformation_string == 'None' and self.Duplicate_Check(image_a, image_b):
                    ...
                    for i in range(1, self.num_options+1):
                        ...
                        if self.Duplicate_Check(image_c, image_d):
                            ...
                            solution_found = True
                            self.Print_Found_Match(answer)
                A_to_B_score = self.PixelCompare(image_a, image_b, transformations_list[t_list_index], transformations_done)
                ...
                if transformation_string == 'None' and self.Duplicate_Check(image_a, image_c):
                    ...
                    for i in range(1, self.num_options+1):
                        ...
                        if self.Duplicate_Check(image_b, image_d):
                            ...
                            solution_found = True
                            self.Print_Found_Match(answer)
                A_to_C_score = self.PixelCompare(image_a, image_c, transformations_list[t_list_index], transformations_done)
                if self.toggleDirection:
                ...
                if A_to_B_score >= A_to_C_score:
                    ...
                    if A_to_B_score >= threshold:
                        ...
                        for i in range(1, self.num_options+1):
                            print('Testing Answer %d . . . ' % i)
                            image_d = self.Load_Image(str(i))
                            C_to_D_score = self.PixelCompare(image_c, image_d, transformations_list[t_list_index], transformations_done)
                        ...
                        print('%f is lower than the the threshold %f . . . ' % (A_to_B_score, threshold))
                else:
                    ...
                    if A_to_C_score >= threshold:
                        ...
                        for i in range(1, self.num_options+1):
                            print('Testing Answer %d . . . ' % i)
                            image_d = self.Load_Image(str(i))
                            B_to_D_score = self.PixelCompare(image_b, image_d, transformations_list[t_list_index], transformations_done)
                        ...
                        print('%f is lower than the the threshold %f . . . ' % (A_to_C_score, threshold))
                self.readyForSecondaryCompare = False
                self.isDirectionSetV = False
                self.isDirectionSetH = False
                self.toggleDirectionV = False
    ...
    def Apply_Difference_3x3_Answer_DPR(self, images):
        ...
        dual_pattern_image = self.OR_images(self.AND_images(im_g, im_h), self.AND_images(im_c, im_f))
    ...
    def Apply_Difference_3x3_Answer_IPR(self, images):
        ...
        for i in range(1, self.num_options+1):
            im_i = self.Load_Image(str(i))
            score = self.PixelCompare_3x3(dual_pattern_image, im_i, transformation)
            # difference = self.Similarity_Check(dual_pattern_image, im_i)
    ...
    def Check_Diagonal_Similarity_3x3_DPR(self, images):
        ...
        if abs(difference_G1) < DIFFERENCE_THRESHOLD and abs(difference_G2) < DIFFERENCE_THRESHOLD:
            ...
            for i, answer in filtered_results:
                ...
                if abs(difference_G3) < DIFFERENCE_THRESHOLD: #score_G3 > DIAGONAL_THRESHOLD_3x3:
            ...
            if results != []:
                if len(results) > 1 and len(results) < 8:
                    ...
                    for ans, sc in results:
                        tiebreaker_results.append((ans, abs(self.Similarity_Check_DPR(dual_pattern_image, self.Load_Image(str(ans))))))
                    print(tiebreaker_results)
                    # best_answer = max(tiebreaker_results)[1]
                    best_result = sorted(tiebreaker_results, key=lambda x: x[1])[0]
                elif len(results) == 8:
        ...
        if abs(difference_G3) < DIFFERENCE_THRESHOLD and abs(difference_G2) < DIFFERENCE_THRESHOLD:
            ...
            for i, answer in filtered_results:
                ...
                if abs(difference_G1) < DIFFERENCE_THRESHOLD: #score_G1 > DIAGONAL_THRESHOLD_3x3:
        ...
        if score_G1 > DIAGONAL_THRESHOLD_3x3 and score_G2 > DIAGONAL_THRESHOLD_3x3:
            ...
            if results != []:
                if len(results) > 1 and len(results) < 8:
                    ...
                    tiebreaker_results = []
                    for ans, sc in results:
                        tiebreaker_results.append((ans, self.Similarity_Check_IPR(dual_pattern_image, self.Load_Image(str(ans)))))
                    print(tiebreaker_results)
                    # best_answer = max(tiebreaker_results)[1]
                    best_result = sorted(tiebreaker_results, key=lambda x: x[1])[-1]
                elif len(results) == 8:
                    print('\nToo many matched results, skipping...')
                    return -1, 0
                else:
        ...
        if score_G3 > DIAGONAL_THRESHOLD_3x3 and score_G2 > DIAGONAL_THRESHOLD_3x3:
            ...
            if results != []:
                ...
                if len(results) > 1 and len(results) < 8:
                    ...
                    tiebreaker_results = []
                    for ans, sc in results:
                        tiebreaker_results.append((ans, abs(self.Similarity_Check_IPR(dual_pattern_image, self.Load_Image(str(ans))))))
                    print(tiebreaker_results)
                    # best_answer = max(tiebreaker_results)[1]
                    best_result = sorted(tiebreaker_results, key=lambda x: x[1])[-1]
                elif len(results) == 8:
    ...
    def Similarity_Check_3x3_DPR(self, image_a, image_b, image_c):
        ...
        score_a_and_b_dpr = 100 * (len(list_a_and_b) - sum(list_a_and_b))/len(list_a_and_b)
    ...
    def Figure_Count_Pattern_3x3_Answer(self):
        ...
        try:
            ...
            if imageData['C'] == FigureCoeff_AC * imageData['A'] and imageData['F'] == FigureCoeff_DF * imageData['D']:
                if ENV == 'dev':
                    print('Figure Count multiplies by %d right to left . . . \n' % FigureCoeff_AC)
                for i in range(1, self.num_options+1):
    ...
    def Get_AND_3x3_Answer_DPR(self, images):
        ...
        Difference_H_ABC = self.Similarity_Check_DPR(self.AND_images(im_a, im_b), im_c)
        Difference_H_DEF = self.Similarity_Check_DPR(self.AND_images(im_d, im_e), im_f)

        Difference_V_ADG = self.Similarity_Check_DPR(self.AND_images(im_a, im_d), im_g)
        Difference_V_BEH = self.Similarity_Check_DPR(self.AND_images(im_b, im_e), im_h)
        ...
        if abs(Difference_H_ABC) < abs(Difference_V_ADG) and abs(Difference_H_DEF) < abs(Difference_V_BEH):
            ...
            if abs(Difference_H_ABC) < abs(DIFFERENCE_THRESHOLD) and abs(Difference_H_DEF) < abs(DIFFERENCE_THRESHOLD):
                ...
                for i in range(1, self.num_options+1):
                    ...
                    difference = self.Similarity_Check_DPR(self.AND_images(im_g, im_h), im_i)
            ...
            if abs(Difference_V_ADG) < DIFFERENCE_THRESHOLD and abs(Difference_V_BEH) < DIFFERENCE_THRESHOLD:
                ...
                for i in range(1, self.num_options+1):
                    ...
                    difference = self.Similarity_Check_DPR(self.AND_images(im_c, im_f), im_i)
                    # if score > AND_THRESHOLD_3x3:
                    if abs(difference) < DIFFERENCE_THRESHOLD:
    ...
    def Get_AND_3x3_Answer_IPR(self, images):
        ...
        Score_H_ABC = self.Similarity_Check_IPR(self.AND_images(im_a, im_b), im_c)
        Score_H_DEF = self.Similarity_Check_IPR(self.AND_images(im_d, im_e), im_f)

        Score_V_ADG = self.Similarity_Check_IPR(self.AND_images(im_a, im_d), im_g)
        Score_V_BEH = self.Similarity_Check_IPR(self.AND_images(im_b, im_e), im_h)
        ...
        if Score_H_ABC > Score_V_ADG:
            ...
            if Score_H_ABC > AND_THRESHOLD_3x3 and Score_H_DEF > AND_THRESHOLD_3x3:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    score = self.Similarity_Check_IPR(self.AND_images(im_g, im_h), im_i)
                    if score > AND_THRESHOLD_3x3:
                        print('-- Score_H_GHI:', score, '--\n')
                        results.append((i, score))
                print(results)
            ...
            if Score_V_ADG > AND_THRESHOLD_3x3 and Score_V_BEH > AND_THRESHOLD_3x3:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    score = self.Similarity_Check_IPR(self.AND_images(im_c, im_f), im_i)
                    if score > AND_THRESHOLD_3x3:
                        print('-- Score_V_CFI:', score, '--\n')
                        results.append((i, score))
                print(results)
    ...
    def Get_OR_3x3_Answer_DPR(self, images):
        ...
        Difference_H_ABC = self.Similarity_Check_DPR(self.OR_images(im_a, im_b), im_c)
        Difference_H_DEF = self.Similarity_Check_DPR(self.OR_images(im_d, im_e), im_f)

        Difference_V_ADG = self.Similarity_Check_DPR(self.OR_images(im_a, im_d), im_g)
        Difference_V_BEH = self.Similarity_Check_DPR(self.OR_images(im_b, im_e), im_h)
        ...
        if abs(Difference_H_ABC) < Difference_V_ADG and abs(Difference_H_DEF) < Difference_V_BEH:
            ...
            if abs(Difference_H_ABC) < DIFFERENCE_THRESHOLD and abs(Difference_H_DEF) < DIFFERENCE_THRESHOLD:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    difference = self.Similarity_Check_DPR(self.OR_images(im_g, im_h), im_i)
                    if abs(difference) < DIFFERENCE_THRESHOLD:
                        print('-- Difference_H_GHI:', difference, '--\n')
                        results.append((i, abs(difference)))
                print(results)
            ...
            if abs(Difference_V_ADG) < DIFFERENCE_THRESHOLD and abs(Difference_V_BEH) < DIFFERENCE_THRESHOLD:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    difference = self.Similarity_Check_DPR(self.OR_images(im_c, im_f), im_i)
                    if abs(difference) < DIFFERENCE_THRESHOLD:
                        print('-- Difference_V_CFI:', difference, '--\n')
                        results.append((i, abs(difference)))
                print(results)
    ...
    def Get_OR_3x3_Answer_IPR(self, images):
        ...
        Score_H_ABC = self.Similarity_Check_IPR(self.OR_images(im_a, im_b), im_c)
        Score_H_DEF = self.Similarity_Check_IPR(self.OR_images(im_d, im_e), im_f)

        Score_V_ADG = self.Similarity_Check_IPR(self.OR_images(im_a, im_d), im_g)
        Score_V_BEH = self.Similarity_Check_IPR(self.OR_images(im_b, im_e), im_h)
        ...
        if Score_H_ABC > Score_V_ADG:
            ...
            if Score_H_ABC > OR_THRESHOLD_3x3 and Score_H_DEF > OR_THRESHOLD_3x3:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    score = self.Similarity_Check_IPR(self.OR_images(im_g, im_h), im_i)
            ...
            if Score_V_ADG > OR_THRESHOLD_3x3 and Score_V_BEH > OR_THRESHOLD_3x3:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    score = self.Similarity_Check_IPR(self.OR_images(im_c, im_f), im_i)
                    if score > OR_THRESHOLD_3x3:
                        print('-- Score_V_CFI:', score, '--\n')
                        results.append((i, score))
                print(results)
    ...
    def Get_Simple_XOR_3x3_Answer_DPR(self, images):
        ...
        Difference_H_ABC = self.Similarity_Check_DPR(self.XOR_images(im_a, im_b), im_c)
        Difference_H_DEF = self.Similarity_Check_DPR(self.XOR_images(im_d, im_e), im_f)

        Difference_V_ADG = self.Similarity_Check_DPR(self.XOR_images(im_a, im_d), im_g)
        Difference_V_BEH = self.Similarity_Check_DPR(self.XOR_images(im_b, im_e), im_h)
        ...
        if abs(Difference_H_ABC) < abs(Difference_V_ADG) and abs(Difference_H_DEF) < abs(Difference_V_BEH):
            ...
            if abs(Difference_H_ABC) < DIFFERENCE_THRESHOLD and abs(Difference_H_DEF) < DIFFERENCE_THRESHOLD:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    difference = self.Similarity_Check_DPR(self.XOR_images(im_g, im_h), im_i)
                    if abs(difference) < DIFFERENCE_THRESHOLD:
                        print('-- Difference_H_GHI:', difference, '--\n')
                        results.append((i, abs(difference)))
                print(results)
            ...
            if abs(Difference_V_ADG) < DIFFERENCE_THRESHOLD and abs(Difference_V_BEH) < DIFFERENCE_THRESHOLD:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    difference = self.Similarity_Check_DPR(self.XOR_images(im_c, im_f), im_i)
                    if abs(difference) < DIFFERENCE_THRESHOLD:
                        print('-- Difference_V_CFI:', difference, '--\n')
                        results.append((i, abs(difference)))
                print(results)
    ...
    def Get_Simple_XOR_3x3_Answer_IPR(self, images):
        ...
        Score_H_ABC = self.Similarity_Check_IPR(self.XOR_images(im_a, im_b), im_c)
        Score_H_DEF = self.Similarity_Check_IPR(self.XOR_images(im_d, im_e), im_f)

        Score_V_ADG = self.Similarity_Check_IPR(self.XOR_images(im_a, im_d), im_g)
        Score_V_BEH = self.Similarity_Check_IPR(self.XOR_images(im_b, im_e), im_h)
        ...
        if Score_H_ABC > Score_V_ADG and Score_H_DEF > Score_V_BEH:
            ...
            if Score_H_ABC > XOR_THRESHOLD_3x3 and Score_H_DEF > XOR_THRESHOLD_3x3:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    score = self.Similarity_Check_IPR(self.XOR_images(im_g, im_h), im_i)
                    if score > XOR_THRESHOLD_3x3:
                        print('-- Score_H_GHI:', score, '--\n')
                        results.append((i, score))
                    else:
            ...
            if Score_V_ADG > XOR_THRESHOLD_3x3 and Score_V_BEH > XOR_THRESHOLD_3x3:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    score = self.Similarity_Check_IPR(self.XOR_images(im_c, im_f), im_i)
                    if score > XOR_THRESHOLD_3x3:
                        print('-- Score_V_CFI:', score, '--\n')
                        results.append((i, score))
                    else:
    ...
    def Get_Subtract_3x3_Answer_DPR(self, images):
        ...
        if abs(Difference_H_ABC) < abs(Difference_V_ADG) and abs(Difference_H_DEF) < abs(Difference_V_BEH):
            ...
            if abs(Difference_H_ABC) < DIFFERENCE_THRESHOLD and abs(Difference_H_DEF) < DIFFERENCE_THRESHOLD:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    list_i = [self.RGB_to_Binary(pixel) for pixel in list(im_i.getdata())]
                    ratio_i_dpr = 100 * (TOTAL_PIXELS - sum(list_i))/TOTAL_PIXELS
                ...
                if results != []:
                    if len(results) > 1:
                        ...
                        Score_H_ABC = self.Similarity_Check_IPR(self.Subtract_images_IPR(im_a, im_b), im_c)
                        Score_H_DEF = self.Similarity_Check_IPR(self.Subtract_images_IPR(im_c, im_e), im_f)

                        Score_V_ADG = self.Similarity_Check_IPR(self.Subtract_images_IPR(im_a, im_d), im_g)
                        Score_V_BEH = self.Similarity_Check_IPR(self.Subtract_images_IPR(im_b, im_e), im_h)
                        ipr_results = []
                        for i, difference in results:
                            im_i = self.Load_Image(str(i))
                            if Score_H_ABC > Score_V_ADG and Score_H_DEF > Score_V_BEH:
                                print('Horizontal is stronger . . . \n')
                                ipr_results.append((i, self.Similarity_Check_IPR(self.Subtract_images_IPR(im_g, im_h), im_i)))
                            else:
                                print('Vertical is stronger . . . \n')
                                ipr_results.append((i, self.Similarity_Check_IPR(self.Subtract_images_IPR(im_c, im_f), im_i)))
                        print(ipr_results)
    ...
    def Get_Subtract_3x3_Answer_IPR(self, images):
        ...
        if Score_H_ABC > Score_V_ADG and Score_H_DEF > Score_V_BEH:
            ...
            if Score_H_ABC > SUBTRACT_THRESHOLD_3x3 and Score_H_DEF > SUBTRACT_THRESHOLD_3x3:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    score = self.Similarity_Check_IPR(self.Subtract_images_IPR(im_g, im_h), im_i)
            ...
            if Score_V_ADG > SUBTRACT_THRESHOLD_3x3 and Score_V_BEH > SUBTRACT_THRESHOLD_3x3:
                ...
                for i in range(1, self.num_options+1):
                    im_i = self.Load_Image(str(i))
                    score = self.Similarity_Check_IPR(self.Subtract_images_IPR(im_c, im_f), im_i)
                    if score > SUBTRACT_THRESHOLD_3x3:
    ...
    def PixelCompare(self, image_a, image_b, transformation, transformations_done):
        if transformation is None:
            ...
            list_a = [self.RGB_to_Binary(pixel) for pixel in list(self.Rotate_Image(image_a, transformation.split(' ')[1]).getdata())]
            list_b = [self.RGB_to_Binary(pixel) for pixel in list(image_b.getdata())]

            score_pixel = 100 * len([i for i, j in zip(list_a, list_b) if i == j]) / len(list_a)
        elif transformation == 'XOR Images':
            image_a_XOR_b = self.XOR_images(image_a, image_b)
            list_a_XOR_b = [self.RGB_to_Binary(pixel) for pixel in list(image_a_XOR_b.getdata())]
            list_a = [self.RGB_to_Binary(pixel) for pixel in list(image_a.getdata())]
            list_b = [self.RGB_to_Binary(pixel) for pixel in list(image_b.getdata())]

            score_pixel_a = 100 * len([i for i, j in zip(list_a_XOR_b, list_a) if i == j]) / len(list_a)
            score_pixel_b = 100 * len([i for i, j in zip(list_a_XOR_b, list_b) if i == j]) / len(list_a)
            ...
            if not self.readyForSecondaryCompare:
                ...
                if score_pixel_a > score_pixel_b:
                    print("Reverse XOR order works better! %f > %f" % (score_pixel_a, score_pixel_b))
                    score_pixel = score_pixel_a
                    # score_rms = self.RMS_Difference(image_a_XOR_b, image_a) / 1000
                    self.toggleDirection = True
                else:
                    print("Standard XOR order works better! %f >= %f" % (score_pixel_b, score_pixel_a))
            ...
            list_a_OR_b = [self.RGB_to_Binary(pixel) for pixel in list(image_a_OR_b.getdata())]
            list_a = [self.RGB_to_Binary(pixel) for pixel in list(image_a.getdata())]
            list_b = [self.RGB_to_Binary(pixel) for pixel in list(image_b.getdata())]

            score_pixel_a = 100 * len([i for i, j in zip(list_a_OR_b, list_a) if i == j]) / len(list_a)
            score_pixel_b = 100 * len([i for i, j in zip(list_a_OR_b, list_b) if i == j]) / len(list_a)
            ...
            if not self.readyForSecondaryCompare:
                ...
                if score_pixel_a > score_pixel_b:
                    print("Reverse OR order works better! %f > %f" % (score_pixel_a, score_pixel_b))
                    score_pixel = score_pixel_a
                    # score_rms = self.RMS_Difference(image_a_OR_b, image_a) / 1000
                    self.toggleDirection = True
                else:
                    print("Standard OR order works better! %f >= %f" % (score_pixel_b, score_pixel_a))
                    score_pixel = score_pixel_b
            ...
            list_a_AND_b = [self.RGB_to_Binary(pixel) for pixel in list(image_a_AND_b.getdata())]
            list_a = [self.RGB_to_Binary(pixel) for pixel in list(image_a.getdata())]
            list_b = [self.RGB_to_Binary(pixel) for pixel in list(image_b.getdata())]

            score_pixel_a = 100 * len([i for i, j in zip(list_a_AND_b, list_a) if i == j]) / len(list_a)
            score_pixel_b = 100 * len([i for i, j in zip(list_a_AND_b, list_b) if i == j]) / len(list_a)
            if not self.readyForSecondaryCompare:
                # if self.problem.name == 'Basic Problem B-10':
                #     image_a_AND_b.show()
                #     image_a.show()
                ...
                if score_pixel_a > score_pixel_b:
                    print("Reverse AND order works better! %f > %f" % (score_pixel_a, score_pixel_b))
                    score_pixel = score_pixel_a
                    # score_rms = self.RMS_Difference(image_a_AND_b, image_a) / 1000
                    self.toggleDirection = True
                else:
                    print("Standard AND order works better! %f > %f" % (score_pixel_b, score_pixel_a))
                    score_pixel = score_pixel_b
            ...
            list_a = [self.RGB_to_Binary(pixel) for pixel in list(image_a.transpose(transformation).getdata())]
            list_b = [self.RGB_to_Binary(pixel) for pixel in list(image_b.getdata())]
    ...
    def Problem_Heading_Printer(self):
        print()
        print('***************************************************')
        print('Solving --', self.problem.name, ', which is --', self.problem.problemType)
        print('***************************************************')
        if ENV == 'dev':
            input()
